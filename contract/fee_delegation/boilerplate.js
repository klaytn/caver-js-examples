...
function loadEnv() {
    ...
    feePayerAddress = feePayerAddress === "" ? result.parsed.FEE_PAYER_ADDRESS : feePayerAddress
    feePayerPrivateKey = feePayerPrivateKey === "" ? result.parsed.FEE_PAYER_PRIVATE_KEY : feePayerPrivateKey
}
...
async function run() {
    ...
    const sender = caver.wallet.keyring.create(senderAddress, senderPrivateKey)
    caver.wallet.add(sender)
    const feePayer = caver.wallet.keyring.create(feePayerAddress, feePayerPrivateKey)
    ...
    // await caver.wallet.signAsFeePayer(feePayer.address, deployTx) // Signs the transaction as a fee payer
    ...
    // await caver.wallet.signAsFeePayer(feePayer.address, executionTx) // Signs the transaction as a fee payer
